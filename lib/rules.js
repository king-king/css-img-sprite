...
var Rules = Object.derive(
    function (id, css) {
        var self = this
            ...
            , __support_size_re = /(\d+px|auto)\s*(\d+px|auto)/i //只支持px
        ...
        self._css = css.replace(__background_re, function (m, image, position, repeat, size) {
                ...
                if (image) {
                    ...
                    if (res && res[1]) {
                        ...
                        if (info.query && __sprites_re.test(info.query) && !info.origin.match(/^https?:\/\//)) {
    ...
    },
    {
        ...
        isSprites: function () {
            // 这里要加上额外的判断
            // 如果存在repeat-x或repeat-y而且还设置了background-size，则为false
            if (this.repeat && this.size[0] != -1) {
                this._is_sprites = false;
            }
